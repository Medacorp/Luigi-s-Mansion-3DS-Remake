execute unless entity @e[tag=boolossus,tag=!dead,limit=1] as @a[tag=!gooigi] run function luigis_mansion:entities/ghost/boss_damage
execute unless entity @e[tag=boolossus,tag=!dead,limit=1] if score #temp Damage matches ..5 run data modify storage luigis_mansion:data current_state.current_data.portrait_ghosts.boolossus.rank set value 3b
execute unless entity @e[tag=boolossus,tag=!dead,limit=1] if score #temp Damage matches 6..19 run data modify storage luigis_mansion:data current_state.current_data.portrait_ghosts.boolossus.rank set value 2b
