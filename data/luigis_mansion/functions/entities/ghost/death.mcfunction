...
execute at @s[scores={HurtTime=1}] unless entity @s[scores={Room=..-1}] if entity @a[tag=killer,limit=1,scores={GhostCount=2..}] run function luigis_mansion:spawn_entities/item/small_heart
execute at @s[scores={HurtTime=1}] unless entity @s[scores={Room=..-1}] if entity @a[tag=killer,limit=1,scores={GhostCount=2..}] run data modify storage luigis_mansion_3ds_remake:data trophy.easy.caught_two_ghosts_at_once set value 1b
execute at @s[scores={HurtTime=1}] run function luigis_mansion:entities/ghost/capture
execute at @s[scores={HurtTime=1,Room=-2}] run scoreboard players add #training GhostCaught 1
