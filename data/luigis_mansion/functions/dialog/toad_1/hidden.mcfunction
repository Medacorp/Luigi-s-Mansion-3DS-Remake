...
execute if entity @s[scores={Dialog=1..1263}] as @a[tag=same_room,gamemode=!spectator,scores={IdleTime=0..},tag=!looking_at_map] run function luigis_mansion:entities/player/animation/set/stand_still
...
execute if entity @s[scores={Dialog=1234}] run scoreboard players set @e[tag=this_model,tag=toad_head,limit=1] AnimationProg 0
...
execute if entity @s[scores={Dialog=1236}] run tellraw @a[tag=same_room] {"translate":"chat.type.text","with":[{"translate":"luigis_mansion:entity.toad","color":"green"},{"translate":"luigis_mansion:dialog.toad_1.yes.3"}]}
...
execute if entity @s[tag=!talk,scores={Dialog=..1061},tag=happy] run scoreboard players set @e[tag=this_model,tag=toad_head,limit=1] AnimationProg 0
