...
execute unless entity @a[scores={TrainingOption=0..}] as @a[tag=same_room] unless entity @s[scores={TrainingOption=-1}] run function #luigis_mansion:dialog/training_room/option
execute unless entity @a[scores={TrainingOption=0..}] as @a[tag=same_room] unless entity @s[scores={TrainingOption=-1}] run scoreboard players set @s TrainingOption -1
execute unless entity @a[scores={TrainingOption=0..}] as @a[tag=same_room] if entity @s[scores={TrainingOption=-1}] run scoreboard players enable @s TrainingOption
