...
execute if entity @s[scores={GBHDialog=1,Variant=..-3}] unless entity @s[scores={Variant=-4}] unless entity @s[scores={Variant=-8..-7}] if score #players Totals matches 1 run tellraw @a {"translate":"chat.type.text","with":[{"translate":"luigis_mansion:entity.e_gadd","color":"green"},{"translate":"luigis_mansion:dialog.caught_boo.extra","with":[{"selector":"@p[gamemode=!spectator]"}]}]}
