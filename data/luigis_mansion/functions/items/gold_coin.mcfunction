...
execute positioned ~ ~1.4 ~ run tag @a[tag=same_room,tag=!spectator,sort=nearest,limit=1] add me
execute unless entity @a[tag=me,limit=1] positioned ~ ~1.4 ~ as @e[tag=same_room,tag=gameboy_horror_location,sort=nearest,limit=1] run function luigis_mansion:entities/gameboy_horror_location/tag_for_money
...
execute unless score #temp ActionTime matches 2147483647 store result storage luigis_mansion:data current_state.my_money.money.gold_coin int 1 run scoreboard players add #temp Time 1
...
execute unless score #temp ActionTime matches 2147483647 store result storage luigis_mansion:data current_state.my_money_data.money.gold_coin int 1 run scoreboard players add #temp Time 1
