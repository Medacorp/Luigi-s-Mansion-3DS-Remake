...
execute as @e[tag=!spectator,x=736,y=28,z=0,dx=24,dy=8,dz=35] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 56
...
execute unless entity @a[tag=toad_5_dialog,limit=1] if data storage luigis_mansion:data current_state.current_data{blackout:1b} unless data storage luigis_mansion:data current_state.current_data.technical_data{telephone_1:1b} run playsound luigis_mansion:block.telephone.ring ambient @a[scores={Room=56,RoomNoise=0}] 750.5 30 17.5 1
...
execute if entity @a[gamemode=!spectator,scores={Room=56}] run function luigis_mansion:room/normal/telephone_room/ghosts

function luigis_mansion:room/normal/door/hallway_19_telephone_room
