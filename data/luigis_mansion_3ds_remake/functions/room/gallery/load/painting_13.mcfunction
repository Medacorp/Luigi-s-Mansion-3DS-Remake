execute if data storage luigis_mansion_3ds_remake:data {selected_portraits:{slim_bankshot:-1b}} run data modify storage luigis_mansion_3ds_remake:data current_state.selected_portraits.slim_bankshot set from storage luigis_mansion:data current_state.portrait_ghost_ranks.slim_bankshot
...
execute positioned 749 13 -28 run tag @e[distance=..0.7,tag=gallery_check] add slim_bankshot_painting