...
execute as @e[tag=!spectator,x=777,y=5,z=-26,dx=51,dy=37,dz=55] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 74
...
execute if entity @a[gamemode=!spectator,scores={Room=74}] run function luigis_mansion_3ds_remake:room/hidden/bogmire_battle/ghosts