...
execute as @e[tag=!spectator,x=741,y=28,z=-17,dx=23,dy=8,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 51
...
execute if entity @a[gamemode=!spectator,scores={Room=51},limit=1] run function luigis_mansion_3ds_remake:room/hidden/hallway_16/ghosts
