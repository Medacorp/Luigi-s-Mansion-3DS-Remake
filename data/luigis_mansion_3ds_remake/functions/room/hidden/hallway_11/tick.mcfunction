...
execute as @e[x=686,y=19,z=-23,dx=10,dy=8,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 37
...
execute if entity @a[gamemode=!spectator,scores={Room=37},limit=1] run function luigis_mansion_3ds_remake:room/hidden/hallway_11/ghosts
