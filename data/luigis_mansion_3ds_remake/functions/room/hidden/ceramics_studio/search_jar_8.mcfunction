...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=poison_mushroom] positioned 710 29 64 run function luigis_mansion:spawn_entities/item/poison_mushroom
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] positioned 710 29 64 run function luigis_mansion_3ds_remake:room/hidden/ceramics_studio/jar_8
...
tag @e[x=711.5,y=29,z=64.5,distance=..0.7,tag=hidden_boo,tag=caught_by_boo_radar] add spawn