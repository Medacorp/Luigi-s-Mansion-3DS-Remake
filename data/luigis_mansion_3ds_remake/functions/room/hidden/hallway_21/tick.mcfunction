...
execute as @e[x=699,y=1,z=-22,dx=13,dy=8,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 60
...
execute if entity @a[gamemode=!spectator,scores={Room=60},limit=1] run function luigis_mansion_3ds_remake:room/hidden/hallway_21/ghosts
