...
execute as @e[x=627,y=16,z=-145,dx=24,dy=11,dz=72] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 46
...
execute if entity @a[gamemode=!spectator,scores={Room=46}] run function luigis_mansion_3ds_remake:room/hidden/observatory/ghosts
