...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=730.0,y=20,z=56.5] if data storage luigis_mansion:data current_state.current_data.obtained_keys{the_twins_room:1b} unless data storage luigis_mansion:data current_state.current_data.used_keys{the_twins_room:1b} positioned 729 20 56 run function luigis_mansion_3ds_remake:room/hidden/the_twins_room/unlock_door
execute if block 729 20 56 #minecraft:doors[open=true] unless data storage luigis_mansion:data current_state.current_data.used_keys{the_twins_room:1b} positioned 729 20 56 run function luigis_mansion:blocks/closed_door
...
execute if score #temp Searched matches 0 run playsound luigis_mansion:block.door.close block @a[scores={Room=7}] 730 20 56 1 1
execute if score #temp Searched matches 0 run playsound luigis_mansion:block.door.close block @a[scores={Room=49}] 737 20 44 1 1
execute if score #temp Searched matches 1 run playsound luigis_mansion:block.door.open block @a[scores={Room=7}] 730 20 56 1 1
execute if score #temp Searched matches 1 run playsound luigis_mansion:block.door.open block @a[scores={Room=49}] 737 20 44 1 1
...
execute if score #temp Searched matches 1..2 as @a[x=729.0,y=20,z=56.0,dx=0,dy=1,dz=0] run teleport @s 738 20 44
execute if score #temp Searched matches 1..2 as @a[x=737.0,y=20,z=44.0,dx=0,dy=1,dz=0] run teleport @s 728 20 56
