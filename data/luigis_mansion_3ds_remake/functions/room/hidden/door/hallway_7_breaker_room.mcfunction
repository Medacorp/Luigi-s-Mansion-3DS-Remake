...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=713.0,y=2,z=-46.5] if data storage luigis_mansion:data current_state.current_data.obtained_keys{breaker_room:1b} unless data storage luigis_mansion:data current_state.current_data.used_keys{breaker_room:1b} positioned 713 2 -47 run function luigis_mansion_3ds_remake:room/hidden/breaker_room/unlock_door
execute if block 713 2 -47 #minecraft:doors[open=true] unless data storage luigis_mansion:data current_state.current_data.used_keys{breaker_room:1b} if data storage luigis_mansion:data current_state.current_data{blackout:1b} positioned 713 2 -47 run function luigis_mansion:blocks/closed_door
execute unless data storage luigis_mansion:data current_state.current_data.used_keys{breaker_room:1b} if data storage luigis_mansion:data current_state.current_data{blackout:1b} if score #temp Searched matches 1..2 run scoreboard players set #temp Searched -1
...
execute if score #temp Searched matches 0 run playsound luigis_mansion:block.door.close block @a[scores={Room=15}] 713.0 2 -47 1 1
execute if score #temp Searched matches 0 run playsound luigis_mansion:block.door.close block @a[scores={Room=57}] 700.0 2 -47 1 1
execute if score #temp Searched matches 1 run playsound luigis_mansion:block.door.open block @a[scores={Room=15}] 713.0 2 -47 1 1
execute if score #temp Searched matches 1 run playsound luigis_mansion:block.door.open block @a[scores={Room=57}] 700.0 2 -47 1 1
...
execute if score #temp Searched matches 1..2 as @a[x=713.0,y=2,z=-47.0,dx=0,dy=1,dz=0] run teleport @s 698 2 -47
execute if score #temp Searched matches 1..2 as @a[x=699.0,y=2,z=-47.0,dx=0,dy=1,dz=0] run teleport @s 714 2 -47
