...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=736.5,y=29,z=-16.0] if data storage luigis_mansion:data current_state.current_data.obtained_keys{the_artists_studio:1b} unless data storage luigis_mansion:data current_state.current_data.used_keys{the_artists_studio:1b} positioned 736 29 -16 run function luigis_mansion_3ds_remake:room/hidden/the_artists_studio/unlock_door
execute if block 736 29 -16 #minecraft:doors[open=true] unless data storage luigis_mansion:data current_state.current_data.used_keys{the_artists_studio:1b} positioned 736 29 -16 run function luigis_mansion:blocks/closed_door
execute unless data storage luigis_mansion:data current_state.current_data.used_keys{the_artists_studio:1b} if score #temp Searched matches 1..2 run scoreboard players set #temp Searched -1
