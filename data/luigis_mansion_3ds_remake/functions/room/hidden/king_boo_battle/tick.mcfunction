...
execute as @e[tag=!spectator,x=465,y=25,z=-40,dx=49,dy=60,dz=85] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 76
...
execute if entity @a[gamemode=!spectator,scores={Room=76}] run function luigis_mansion_3ds_remake:room/hidden/king_boo_battle/ghosts