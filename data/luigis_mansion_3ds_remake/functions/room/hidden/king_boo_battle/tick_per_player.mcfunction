...
execute if data storage luigis_mansion:data current_state.current_data.portrait_ghosts.king_boo{health:0} unless entity @s[scores={MusicType=14}] unless entity @s[scores={MusicType=-1}] run function luigis_mansion:other/music/set/area_boss_defeated
...
execute if data storage luigis_mansion:data current_state.current_data.rooms.secret_altar{cleared:1b} run function luigis_mansion_3ds_remake:room/hidden/boolossus_battle/clear_secret_altar