...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=poison_mushroom] positioned 756 31 63 run function luigis_mansion:spawn_entities/item/room_search/poison_mushroom
...
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] positioned 734 29 64 run function luigis_mansion:blocks/dust_no_sound
...
tag @e[x=756.5,y=29,z=64.5,distance=..0.7,tag=hidden_boo,tag=caught_by_boo_radar] add spawn