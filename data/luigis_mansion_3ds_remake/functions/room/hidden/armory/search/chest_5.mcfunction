...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] positioned 746 30 63.0 run function luigis_mansion_3ds_remake:room/hidden/armory/chest_5
...
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] positioned 744 29 63.0 run function luigis_mansion:blocks/dust_no_sound
...
tag @e[x=746.5,y=29,z=63.5,distance=..0.7,tag=hidden_boo,tag=caught_by_boo_radar] add spawn