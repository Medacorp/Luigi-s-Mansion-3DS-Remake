...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] positioned 750.0 30 74 run function luigis_mansion_3ds_remake:room/hidden/armory/chest_1
...
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] positioned 741.0 29 74 run function luigis_mansion:blocks/dust_no_sound
...
tag @e[x=750.5,y=29,z=74.5,distance=..0.7,tag=hidden_boo,tag=caught_by_boo_radar] add spawn