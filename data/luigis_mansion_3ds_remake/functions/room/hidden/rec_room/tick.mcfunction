...
execute as @e[tag=!spectator,x=637,y=10,z=-62,dx=12,dy=8,dz=33] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 32
...
execute if entity @a[gamemode=!spectator,scores={Room=32},limit=1] run function luigis_mansion_3ds_remake:room/hidden/rec_room/ghosts
