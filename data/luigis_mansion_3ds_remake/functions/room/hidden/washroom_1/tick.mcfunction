...
execute as @e[tag=!spectator,x=660,y=10,z=34,dx=8,dy=8,dz=14] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 19
...
execute if entity @a[gamemode=!spectator,scores={Room=19}] run function luigis_mansion_3ds_remake:room/hidden/washroom_1/ghosts