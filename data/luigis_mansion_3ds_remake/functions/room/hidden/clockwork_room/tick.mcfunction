...
execute as @e[tag=!spectator,x=682,y=119,z=-10,dx=17,dy=8,dz=35] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 62
...
execute if entity @a[gamemode=!spectator,scores={Room=62},limit=1] run function luigis_mansion_3ds_remake:room/hidden/clockwork_room/ghosts
