execute unless score #the_artists_studio Ticking matches 1 run function #luigis_mansion_3ds_remake:room/hidden/the_artists_studio/load
...
execute as @e[tag=!spectator,x=687,y=28,z=-80,dx=16,dy=8,dz=25] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 71
scoreboard players set #temp Room 71

execute as @a[gamemode=!spectator,scores={Room=71}] run function luigis_mansion_3ds_remake:room/hidden/the_artists_studio/tick_per_player
...
execute as @e[tag=!spectator,x=697.0,y=28,z=-78.0,dx=5,dy=8,dz=21,tag=ghost,tag=hidden,y_rotation=-180..0] at @s run function luigis_mansion_3ds_remake:room/hidden/the_artists_studio/stay_near_painting
...
function #luigis_mansion_3ds_remake:room/hidden/the_artists_studio/interactions/room

execute if entity @a[gamemode=!spectator,scores={Room=71}] run function luigis_mansion_3ds_remake:room/hidden/the_artists_studio/ghosts

function luigis_mansion_3ds_remake:room/hidden/door/hallway_17_the_artists_studio