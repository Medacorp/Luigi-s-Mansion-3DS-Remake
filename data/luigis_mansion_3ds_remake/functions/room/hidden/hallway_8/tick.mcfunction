...
execute if entity @a[advancements={luigis_mansion:keys={hallway_8=false}},limit=1] run function luigis_mansion_3ds_remake:room/hidden/hallway_8/lock_door
...
execute as @e[type=!minecraft:item_frame,x=659,y=10,z=-62,dx=8,dy=17,dz=26] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 33
