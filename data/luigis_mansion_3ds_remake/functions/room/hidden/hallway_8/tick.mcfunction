...
execute if entity @e[x=661.0,y=20,z=-43.5,distance=..0.7,type=minecraft:villager,tag=door,tag=!burning,limit=1] run data modify storage luigis_mansion:data current_state.current_data.technical_data merge value {extinguished_hallway_8_fire:1b}
function #luigis_mansion_3ds_remake:room/hidden/hallway_8/interactions/room

scoreboard players set #temp Room 33
execute as @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door] run function #luigis_mansion:get_same_room
scoreboard players reset #temp Room
execute if entity @a[tag=same_room,limit=1] run function luigis_mansion_3ds_remake:room/hidden/hallway_8/ghosts
