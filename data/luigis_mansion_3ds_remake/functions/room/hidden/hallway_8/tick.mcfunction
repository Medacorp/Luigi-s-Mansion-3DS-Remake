...
execute as @e[tag=!spectator,x=659,y=10,z=-62,dx=8,dy=17,dz=26] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 33
...
execute if entity @a[gamemode=!spectator,scores={Room=33},limit=1] run function luigis_mansion_3ds_remake:room/hidden/hallway_8/ghosts
