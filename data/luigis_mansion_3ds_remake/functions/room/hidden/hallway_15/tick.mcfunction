...
execute as @e[tag=!spectator,x=713,y=28,z=-42,dx=7,dy=8,dz=13] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 41
...
execute if entity @a[gamemode=!spectator,scores={Room=41},limit=1] run function luigis_mansion_3ds_remake:room/hidden/hallway_15/ghosts
