...
execute as @e[tag=!spectator,x=673,y=10,z=64,dx=15,dy=8,dz=27] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 47
...
execute if entity @a[gamemode=!spectator,scores={Room=47},limit=1] run function luigis_mansion_3ds_remake:room/hidden/billiards_room/ghosts

function luigis_mansion_3ds_remake:room/hidden/door/hallway_5_billiards_room
