...
execute as @e[tag=!spectator,x=697,y=10,z=28,dx=14,dy=8,dz=30] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 26
...
execute if entity @a[gamemode=!spectator,scores={Room=26}] run function luigis_mansion_3ds_remake:room/hidden/dining_room/ghosts