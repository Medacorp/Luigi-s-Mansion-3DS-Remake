execute if data storage luigis_mansion:data current_state.current_data.rooms.conservatory{cleared:1b} if entity @s[tag=!blackout] unless entity @s[scores={MusicGroup=0,MusicType=9}] unless entity @s[scores={MusicGroup=0,MusicType=23}] unless entity @s[scores={MusicGroup=0,MusicType=25}] unless entity @s[scores={MusicGroup=0,MusicType=14}] unless entity @s[scores={MusicGroup=0,MusicType=19}] unless entity @s[scores={MusicGroup=0,MusicType=27..30}] unless entity @s[scores={MusicGroup=0,MusicType=-1..0}] run function luigis_mansion:other/music/set/light
...
execute if entity @s[tag=blackout] unless entity @s[scores={MusicGroup=0,MusicType=7}] unless entity @s[scores={MusicGroup=0,MusicType=23}] unless entity @s[scores={MusicGroup=0,MusicType=25}] unless entity @s[scores={MusicGroup=0,MusicType=14}] unless entity @s[scores={MusicGroup=0,MusicType=19}] unless entity @s[scores={MusicGroup=0,MusicType=27..30}] unless entity @s[scores={MusicGroup=0,MusicType=-1..0}] run function luigis_mansion:other/music/set/silence
title @s[tag=!seen_room_name] title {"translate":"luigis_mansion:location.conservatory"}
tag @s add seen_room_name

execute if data storage luigis_mansion:data current_state.current_data.rooms.conservatory{seen:0b} run function luigis_mansion_3ds_remake:room/hidden/conservatory/set_seen
...
execute if score #conservatory_harp Searched matches 1 if entity @s[scores={Time=0,MusicType=0}] run particle minecraft:note 657.0 14 -6.0 0 0 0 1 1 normal @s
