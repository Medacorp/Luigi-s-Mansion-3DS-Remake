...
execute as @e[x=660,y=19,z=34,dx=8,dy=8,dz=14] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 42
...
execute if entity @a[gamemode=!spectator,scores={Room=42}] run function luigis_mansion_3ds_remake:room/hidden/washroom_2/ghosts
