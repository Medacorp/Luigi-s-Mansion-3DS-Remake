execute unless score #bottom_of_the_well Ticking matches 1 run function #luigis_mansion_3ds_remake:room/hidden/bottom_of_the_well/load
execute as @a[gamemode=!spectator,x=645,y=92,z=-10,dx=6,dy=7,dz=11] unless entity @s[scores={Room=31}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[type=!minecraft:item_frame,x=645,y=92,z=-10,dx=6,dy=7,dz=11] run scoreboard players set @s Room 31
...
execute as @a[gamemode=!spectator,scores={Room=31}] run function luigis_mansion_3ds_remake:room/hidden/bottom_of_the_well/tick_per_player

function #luigis_mansion_3ds_remake:room/hidden/bottom_of_the_well/interactions/room

function luigis_mansion_3ds_remake:room/hidden/bottom_of_the_well/ghosts
