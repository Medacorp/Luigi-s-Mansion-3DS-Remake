...
execute as @e[tag=!spectator,x=644,y=101,z=-41,dx=10,dy=8,dz=79] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 30
...
execute if entity @a[gamemode=!spectator,scores={Room=30},limit=1] run function luigis_mansion_3ds_remake:room/hidden/courtyard/ghosts
