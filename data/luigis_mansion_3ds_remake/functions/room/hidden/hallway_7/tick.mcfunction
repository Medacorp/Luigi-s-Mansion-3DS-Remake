...
execute as @e[x=712,y=1,z=-63,dx=8,dy=17,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 15
...
execute if entity @a[gamemode=!spectator,scores={Room=15},limit=1] run function luigis_mansion_3ds_remake:room/hidden/hallway_7/ghosts
