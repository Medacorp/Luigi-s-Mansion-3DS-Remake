...
execute as @e[tag=!spectator,x=634,y=93,z=-27,dx=20,dy=7,dz=18] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 72
...
execute as @e[tag=!spectator,x=636.5,y=94,z=-17.5,distance=..0.7,type=minecraft:item_frame,limit=1] run kill @s[nbt=!{Item:{id:"minecraft:painting"}}]
...
execute if entity @a[gamemode=!spectator,scores={Room=72}] run function luigis_mansion_3ds_remake:room/hidden/secret_altar/ghosts
