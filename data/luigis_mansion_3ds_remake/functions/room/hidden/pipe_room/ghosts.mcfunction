...
execute unless data storage luigis_mansion:data current_state.current_data.rooms.pipe_room{cleared:1b} if entity @a[tag=!blackout] unless score #pipe_room Wave matches 1.. run function luigis_mansion_3ds_remake:room/hidden/pipe_room/wave_1
execute if entity @a[tag=blackout] unless score #pipe_room Wave matches 1.. run function luigis_mansion_3ds_remake:room/hidden/pipe_room/blackout
