execute unless score #pipe_room Ticking matches 1 run function #luigis_mansion_3ds_remake:room/hidden/pipe_room/load
...
execute as @e[x=686,y=1,z=-6,dx=14,dy=8,dz=18] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 67
scoreboard players set #temp Room 67

execute as @a[gamemode=!spectator,scores={Room=67}] run function luigis_mansion_3ds_remake:room/hidden/pipe_room/tick_per_player
...
function #luigis_mansion_3ds_remake:room/hidden/pipe_room/interactions/room

execute if entity @a[gamemode=!spectator,scores={Room=67}] run function luigis_mansion_3ds_remake:room/hidden/pipe_room/ghosts
