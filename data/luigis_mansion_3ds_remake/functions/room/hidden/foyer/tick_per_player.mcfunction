...
execute if entity @s[x=751.0,y=11,z=9.0,distance=..3,tag=!chance_check] unless entity @a[x=751.0,y=11,z=9.0,distance=..3,tag=scanning,limit=1] unless entity @a[tag=warp,limit=1] if data storage luigis_mansion:data {found_e_gadd:1b,current_state:{current_data:{rooms:{nursery:{cleared:0b}}}}} unless entity @e[scores={Room=1},tag=chandelier,type=minecraft:marker,limit=1] run function luigis_mansion_3ds_remake:room/hidden/foyer/falling_chandelier
