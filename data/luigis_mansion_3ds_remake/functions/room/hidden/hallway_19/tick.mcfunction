...
execute as @e[type=!minecraft:item_frame,x=740,y=28,z=40,dx=23,dy=8,dz=8] unless entity @s[type=!minecraft:player,tag=!model_piece,scores={Room=-2147483648..}] run scoreboard players set @s Room 55
tag @e[tag=ghost,scores={Room=55}] add no_hidden_move

execute as @a[gamemode=!spectator,scores={Room=55}] run function luigis_mansion_3ds_remake:room/hidden/hallway_19/tick_per_player

function #luigis_mansion_3ds_remake:room/hidden/hallway_19/interactions/room
...
function luigis_mansion_3ds_remake:room/hidden/hallway_19/ghosts
