...
execute as @e[tag=!spectator,x=667,y=19,z=4,dx=11,dy=8,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 36
...
execute if entity @a[gamemode=!spectator,scores={Room=36},limit=1] run function luigis_mansion_3ds_remake:room/hidden/hallway_10/ghosts
