...
execute as @e[x=737,y=10,z=53,dx=14,dy=8,dz=20] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 23
...
execute if entity @a[gamemode=!spectator,scores={Room=23},limit=1] run function luigis_mansion_3ds_remake:room/hidden/butlers_room/ghosts
