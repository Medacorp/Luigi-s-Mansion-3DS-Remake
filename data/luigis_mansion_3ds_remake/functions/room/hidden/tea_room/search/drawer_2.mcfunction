...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] positioned 643 21 -59 run function luigis_mansion_3ds_remake:room/hidden/tea_room/drawer_2
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] positioned 643 20 -60 run function luigis_mansion:blocks/dust
...
tag @e[x=643.5,y=20,z=-59.5,distance=..0.7,tag=hidden_boo,tag=caught_by_boo_radar] add spawn
