...
execute unless data storage luigis_mansion:data current_state.current_data.rooms.tea_room{cleared:1b} if entity @a[tag=!blackout] unless score #tea_room Wave matches 1.. run function luigis_mansion_3ds_remake:room/hidden/tea_room/wave_1
execute if entity @a[tag=blackout] unless score #tea_room Wave matches 1.. run function luigis_mansion_3ds_remake:room/hidden/tea_room/blackout
