...
execute as @e[x=695,y=10,z=-70,dx=17,dy=8,dz=33] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 17
...
execute if entity @a[gamemode=!spectator,scores={Room=17},limit=1] run function luigis_mansion_3ds_remake:room/hidden/ball_room/ghosts
