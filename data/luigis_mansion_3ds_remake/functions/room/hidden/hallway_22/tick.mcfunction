...
execute as @e[tag=!spectator,x=653,y=1,z=-24,dx=33,dy=8,dz=12] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 61
...
execute if entity @a[gamemode=!spectator,scores={Room=61},limit=1] run function luigis_mansion_3ds_remake:room/hidden/hallway_22/ghosts
