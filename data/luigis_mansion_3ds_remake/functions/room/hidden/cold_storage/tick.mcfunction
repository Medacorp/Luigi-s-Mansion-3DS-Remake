...
execute as @e[x=721,y=1,z=-6,dx=21,dy=8,dz=18] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 68
...
execute if entity @a[gamemode=!spectator,scores={Room=68},limit=1] run function luigis_mansion_3ds_remake:room/hidden/cold_storage/ghosts
