...
tag @s add me
execute as @e[type=minecraft:marker,tag=weak_flashlight,tag=!me] if score @s PathStep = #temp Time if score @s HomeRotX = #temp HomeRotX if score @s HomeRotY = #temp HomeRotY if score @s HomeX = #temp HomeX if score @s HomeY = #temp HomeY if score @s HomeZ = #temp HomeZ run tag @e[type=minecraft:marker,tag=me,limit=1] add ignore
tag @s remove me
