...
execute unless entity @s[scores={HomeX=0,HomeZ=0}] at @s[tag=!looking_at_map] positioned ^ ^-1.65 ^.25 rotated as @e[tag=gooigi,limit=1] rotated ~ 0 positioned ^ ^ ^-0.5 if entity @e[tag=gooigi,limit=1,distance=..0.49] run tag @s add moving_backwards
execute unless entity @s[scores={HomeX=0,HomeZ=0}] at @s[tag=!looking_at_map,tag=!moving_backwards] facing entity @e[tag=gooigi,limit=1] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
execute at @s[tag=!looking_at_map,tag=moving_backwards] rotated as @e[tag=gooigi,limit=1] rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
execute unless entity @s[tag=!looking_at_map,scores={HomeX=0,HomeZ=0}] store result score @s PlayerRotation run data get entity @s Rotation[0] 1000
...
# Move with
execute rotated as @s run teleport @s[tag=!looking_at_map,tag=!flipped_gravity,tag=!sneak_pos,nbt=!{Small:1b}] ^ ^1.65 ^
execute rotated as @s run teleport @s[tag=!looking_at_map,tag=!flipped_gravity,tag=sneak_pos,nbt=!{Small:1b}] ^ ^1.4 ^
execute rotated as @s run teleport @s[tag=!looking_at_map,tag=!flipped_gravity,tag=!sneak_pos,nbt={Small:1b}] ^ ^1.15 ^
execute rotated as @s run teleport @s[tag=!looking_at_map,tag=!flipped_gravity,tag=sneak_pos,nbt={Small:1b}] ^ ^1 ^
execute rotated as @s run teleport @s[tag=!looking_at_map,tag=flipped_gravity,tag=!sneak_pos,nbt=!{Small:1b}] ^ ^-0.6 ^
execute rotated as @s run teleport @s[tag=!looking_at_map,tag=flipped_gravity,tag=sneak_pos,nbt=!{Small:1b}] ^ ^-0.35 ^
execute rotated as @s run teleport @s[tag=!looking_at_map,tag=flipped_gravity,tag=!sneak_pos,nbt={Small:1b}] ^ ^-0.3 ^
execute rotated as @s run teleport @s[tag=!looking_at_map,tag=flipped_gravity,tag=sneak_pos,nbt={Small:1b}] ^ ^-0.3 ^
