execute positioned ~-0.5 ~-0.5 ~-0.5 run tag @e[dx=0,dy=0,dz=0,tag=!scan_ignore,tag=!scanning_player,tag=!spectator,type=!minecraft:item_frame,limit=1] add target
execute if score #can_warp Selected matches 1 if block ~ ~ ~ #luigis_mansion:gameboy_horror_warp unless entity @e[tag=target,limit=1] run tag @s add warp
