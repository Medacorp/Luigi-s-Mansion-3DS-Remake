...
execute if entity @s[tag=flashlight] positioned ~ ~1 ~ positioned ^ ^ ^1 if block ~ ~ ~ #luigis_mansion:flashlight_path run summon minecraft:marker ~ ~ ~ {Tags:["weak_flashlight","new"]}
...
execute if entity @s[tag=flashlight] as @e[type=minecraft:marker,tag=flashlight,tag=new,limit=1] at @s run function luigis_mansion_3ds_remake:items/flashlight/effects/weak_light/path
